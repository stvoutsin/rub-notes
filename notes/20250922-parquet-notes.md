...
This seems to be expected behaviour in pyarrow, so it seems that the usage pattern we are seeing is pyarrow holding on to memory, rather than a memory leak:
...
A few relevant messages from this arrow issue:
